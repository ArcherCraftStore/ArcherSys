...
<!-- MODX by the phpBB MOD Team XSL file v1.0 copyright 2005-2007 the phpBB MOD Team. 
	$Id: modx.prosilver.en.xsl,v 1.1 2007/05/08 10:17:50 smithy_dll Exp $ -->
		...
		if(xhr.readyState == 4)
		{
			...
			{   
				...
				for (i in languages)
				{
					...
					{
						if (documentLanguages[l].match(languages[i][0]))
						{
							var langItem = document.createElement('option');                            
							langSel.appendChild(langItem);
							langItem.value = documentLanguages[l];
							var iso = documentLanguages[l].split('-');
