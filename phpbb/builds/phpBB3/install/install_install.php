...
*
* @package install
* @version $Id: install_install.php,v 1.172 2007/07/27 17:33:15 acydburn Exp $
...
//		$config_data .= "\$acm_type = '" . (($acm_type) ? $acm_type : 'file') . "';\n";
	...
	*/
	function obtain_advanced_settings($mode, $sub)
	{
		...
		if (
			(
				$_port = (int)array_pop(explode(':', $_SERVER['HTTP_HOST']))
			) != $data['server_port']
			&&
			$_port > 0
		) {
			$data['server_port'] = $_port;
		}
		...
		$data['script_path'] = str_replace('/builds/phpBB3', '/', $data['script_path']);
	...
	*/
	function load_schema($mode, $sub)
	{
		...
		$sql_ary = array(
			...
			'UPDATE ' . $data['table_prefix'] . "config
				SET config_value = 'images/avatars'
