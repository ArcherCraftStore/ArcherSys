...
class Actions extends ActionsBase
{
	...
	public function GetAccount($iAccountId)
	{
		...
		if (0 < $iUserId)
		{
			...
			if ($oAccount instanceof \CAccount && $oAccount->IdUser === $iUserId && !$oAccount->IsDisabled)
			{
				$oResult = $oAccount;
			}
		}
		...
		if ($oAccount)
		{
			...
			if (!empty($sUrl))
			{
				if (\api_Utils::GetLinkType($sUrl) === \EFileStorageLinkType::GoogleDrive)
				{
					...
					$oInfo = \api_Utils::GetGoogleDriveFileInfo($sUrl, $oTenant->SocialGoogleApiKey);
					if ($oInfo)
					{
						$mResult['Size'] = 0;
				...
				else
				{
                                        ...
                                        if (\api_Utils::RemoteFileExists($sUrl))
					{
						...
						$mResult['Name'] = urldecode($sFileName);
						$mResult['Size'] = $aRemoteFileInfo['size'];
					}
				}
			}
	 ...
	 * @return array
	 */
	public function AjaxEmailDomainData()
	{
		$oAccount = $this->getAccountFromParam();
	 ...
	 * @return array
	 */
	public function AjaxExpandAttachment()
	{
		$self = $this;
		$mResult = false;
		$oAccount = null;
					...
					foreach ($aExpand as $aItem)
					{
						if ($aItem && isset($aItem['FileName'], $aItem['MimeType'], $aItem['Size'], $aItem['TempName']))
						{
							$mResult[] = array(
								...
								'Expand' => $self->isExpandMimeTypeSupported($aItem['MimeType'], $aItem['FileName']),
								'IsInline' => false,
								'IsLinked' => false,
	 ...
	 * @return array
	 */
	public function AjaxMessageFlags()
	{
		$sFolderFullNameRaw = (string) $this->getParamValue('Folder', '');
		$aUids = $this->getParamValue('Uids', array());
	 ...
	 * @return array
	 */
	public function AjaxMessageZipAttachments()
	{
	...
	public function AjaxMessageSend()
	{
		...
		if ($oMessage)
		{
			...
			catch (\CApiManagerException $oException)
			{
				...
				throw new \ProjectSeven\Exceptions\ClientException($iCode, $oException, $oException->GetPreviousMessage());
			}
									...
									$self->ApiFileCache()->PutFile($oDefAccount, $sTempName, $rResource))
								{
									...
									if (is_resource($rSubResource))
									{
										$mResult[$sAttachment] = $oApiFilestorage->CreateFile(
											$oDefAccount, \EFileStorageType::Personal, '', $sFileName, $rSubResource, false);
									}
	 ...
	 * @return bool
	 */
	public function AjaxPdfFromHtml()
	{
		$oAccount = $this->getAccountFromParam();
		if ($oAccount)
		{
...
//		$sLang = $this->validateLang($sLang);
	 ...
	 * @return array
	 */
	public function AjaxGetForgotAccountQuestion()
	{
		$sEmail = trim((string) $this->getParamValue('Email', ''));
	 ...
	 * @return array
	 */
	public function AjaxValidateForgotAccountQuestion()
	{
		$sEmail = trim((string) $this->getParamValue('Email', ''));
		$sQuestion =  trim((string) $this->getParamValue('Question', ''));
		$sAnswer =  trim((string) $this->getParamValue('Answer', ''));
			...
			if (is_array($aLinks) && 0 < count($aLinks))
			{
				...
				foreach ($aLinks as $sLink)
				{
					...
					if ($sLink)
					{
						...
						{
							$oInfo = \api_Utils::GetGoogleDriveFileInfo($sLink, $oTenant->SocialGoogleApiKey, $sAccessToken, $bLinksAsIds);
							if ($oInfo)
							{
				...
				foreach ($aFiles as $sHash)
				{
					...
					if (\is_array($aData) && 0 < \count($aData))
					{
						...
						if ($oFileInfo->IsLink)
						{
							...
							{
								$oInfo = \api_Utils::GetGoogleDriveFileInfo($oFileInfo->LinkUrl, $oTenant->SocialGoogleApiKey);
								$aData['Name'] = isset($oInfo->title) ? $oInfo->title : $aData['Name'];
								$aData['Size'] = isset($oInfo->fileSize) ? $oInfo->fileSize : $aData['Size'];
	 ...
	 * @return array
	 */
	public function AjaxGetTwilioLogs()
	{
		$oAccount = $this->getAccountFromParam();
		...
		if ($oTenant && $this->oApiCapability->IsTwilioSupported($oAccount) &&
...
//				$oCapability->allowClientIncoming('TwilioAftId_'.$oAccount->IdTenant.'_'.$oAccount->User->TwilioNumber);
	 ...
	 * @return array
	 */
	public function AjaxContactByEmails()
	{
		$aResult = array();
		$oAccount = $this->getDefaultAccountFromParam();
	...
	public function AjaxUpdateAttendeeStatus()
	{
		$oAccount = $this->getAccountFromParam();
	 ...
	 * @return array
	 */
	public function AjaxGroupEvents()
	{
		$aEvents = array();
		$oAccount = $this->getDefaultAccountFromParam();
	...
	public function AjaxContactSuggestions()
	{
		...
		/*$bAllowContactsSharing = (bool) \CApi::GetConf('labs.contacts-sharing', false);
		if ($bAllowContactsSharing && !$bPhoneOnly)
		{
	...
	public function AjaxContactList()
	{
		...
		if ($this->oApiCapability->IsPersonalContactsSupported($oAccount))
		{
			...
			$iCount = $oApiContacts->GetContactItemsCount(
					$oAccount->IdUser, $sSearch, $sFirstCharacter, $iGroupId, 
					$iTenantId);
			...
			if (0 < $iCount)
			{
				$aContacts = $oApiContacts->GetContactItems(
					...
					$iLimit, $sSearch, $sFirstCharacter, $sGroupId, $iTenantId);
	 ...
	 * @return array
	 */
	public function AjaxGetIdentities()
	{
		$oAccount = $this->getAccountFromParam();
	 ...
	 * @return array
	 */
	public function AjaxCreateIdentity()
	{
		$oAccount = $this->getAccountFromParam();
		$sEmail = trim((string) $this->getParamValue('Email', ''));
	 ...
	 * @return array
	 */
	public function AjaxDeleteIdentity()
	{
		$oAccount = $this->getAccountFromParam();
	...
	public function AjaxContactUpdateSharedToAll()
	{
		...
		if ($oApiContacts && $bAllowContactsSharing)
		{
			foreach ($aContactsId as $sContactId)
			{
				...
				if ($oContact)
				{
					...
					{
						if ($oApiContacts->UpdateContactUserId($oContact, $oAccount->IdUser))
						{
							$oContact->IdUser = $oAccount->IdUser;
						}
					}
	...
	public function AjaxFiles()
	{
		...
		$oResult = array(
			'Items' => $this->oApiFilestorage->GetFiles($oAccount, $iType, $sPath, $sPattern),
	...
	public function AjaxFilesLinkCreate()
	{
		...
		$oResult = $this->oApiFilestorage->CreateLink($oAccount, $iType, $sPath, $sLink, $sName);
	...
	public function AjaxFilesCreatePublicLink()
	{
		...
		$mResult = $this->oApiFilestorage->CreatePublicLink($oAccount, $iType, $sPath, $sName, $sSize, $bIsFolder);
		
		return $this->DefaultResponse($oAccount, __FUNCTION__, $mResult);
	}	
	
	public function AjaxFilesDeletePublicLink()
	{
		$mResult = false;
		$oAccount = $this->getDefaultAccountFromParam();
		if (!$this->oApiCapability->IsFilesSupported($oAccount))
		...
		$mResult = $this->oApiFilestorage->DeletePublicLink($oAccount, $iType, $sPath, $sName);
		return $this->DefaultResponse($oAccount, __FUNCTION__, $mResult);
	}
...
//		print_r($aValues);
	...
	public function UploadCalendars()
	{
		...
		return $this->DefaultResponse($oAccount, 'UploadCalendars', $aResponse);
	}	
	...
	public function UploadFile()
	{
		...
		if ($oAccount)
		{
			if (is_array($aFileData))
			{
				...
				$iType = isset($aAdditionalData['Type']) ? (int) $aAdditionalData['Type'] : \EFileStorageType::Personal;
				...
				if ($this->ApiFileCache()->MoveUploadedFile($oAccount, $sSavedName, $aFileData['tmp_name']))
				{
					...
					$this->oApiFilestorage->CreateFile($oAccount, $iType, $sPath, $sUploadName, $rData, false);
...
//		$sLanguage = $this->validateLang($sLanguage);
				...
				foreach ($mAttachments as $sTempName => $sHash)
				{
					...
					if ($rData)
					{
						...
						$this->oApiFilestorage->CreateFile($oUser,
							\EFileStorageType::Corporate, $sThreadFolderName, $sUploadName, $rData, false);
	...
	public function AjaxSocialAccount()
	{
		$mResult = false;
		$oTenant = null;
		$oAccount = $this->GetDefaultAccount();
		...
		{
			$oApiSocial /* @var $oApiSocial \CApiSocialManager */ = \CApi::Manager('social');
			$mResult = $oApiSocial->GetSocial($oAccount->IdAccount, $iType);
		}
		return $this->DefaultResponse(null, __FUNCTION__, $mResult);
	}	
