...
class CContact extends api_AContainer
{
	...
	public function InitFromVCardObject($iUserId, $oVCardObject)
	{
		if ($oVCardObject)
		{
			$sUid = (isset($oVCardObject->UID)) ? (string)$oVCardObject->UID . '.vcf' : \Sabre\VObject\UUIDUtil::getUUID() . '.vcf';
			...
			if (isset($oVCardObject->FN))
			{
				$this->FullName = (string)$oVCardObject->FN;
			}
			...
			if (isset($oVCardObject->NICKNAME))
			{
				$this->NickName = (string) $oVCardObject->NICKNAME;
			}

			if (isset($oVCardObject->NOTE))
			{
				$this->Notes = (string) $oVCardObject->NOTE;
			}
			...
			if (isset($oVCardObject->TITLE))
			{
				$this->BusinessJobTitle = (string)$oVCardObject->TITLE;
			}
			...
			if (isset($oVCardObject->EMAIL))
			{
				...
				foreach($oVCardObject->EMAIL as $oEmail)
				{
					if ($oType = $oEmail['TYPE'])
					{
						...
						{
							if (isset($oVCardObject->{'X-ABLabel'}))
							{
								$aABLabels = $oVCardObject->{'X-ABLabel'};
