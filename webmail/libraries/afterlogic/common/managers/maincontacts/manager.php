...
class CApiMaincontactsManager extends AApiManagerWithStorage
{
	 ...
	 * @return int
	 */
	public function GetContactItemsCount($iUserId, $sSearch = '', $sFirstCharacter = '', $iGroupId = 0, $iTenantId = null)
	{
		$iResult = 0;
		try
		{
			$iResult = $this->oStorage->GetContactItemsCount($iUserId, $sSearch, $sFirstCharacter, $iGroupId, $iTenantId);
		}
		catch (CApiBaseException $oException)
		{
			$iResult = 0;
	...
	public function GetContactItems($mUserId,
		...
		$iOffset = 0, $iRequestLimit = 20, $sSearch = '', $sFirstCharacter = '', $mGroupId = '', $iTenantId = null)
	{
		$mResult = false;
		try
		{
			$mResult = $this->oStorage->GetContactItems($mUserId, $iSortField, $iSortOrder,
				$iOffset, $iRequestLimit, $sSearch, $sFirstCharacter, $mGroupId, $iTenantId);
		}
		catch (CApiBaseException $oException)
		{
			$mResult = false;
		...
		try
		{
			...
			if (!$bGlobalOnly && $oApiCapaManager->IsPersonalContactsSupported($oAccount))
			{
				...
				$aContactItems = $this->oStorage->GetSuggestContactItems($oAccount->IdUser, 
					$sSearch, $iRequestLimit, $bPhoneOnly, $iSharedTenantId);
				...
				if (is_array($aContactItems))
				{
					...
					{
						$sEmail = $oItem->Email;
						if (!empty($sEmail) && !$oItem->Auto)
						{
							$aEmails[$sEmail] = $oItem->Frequency;
						}
					}
					...
					{
						$sEmail = $oItem->Email;
						if (!$oItem->Auto || !isset($aEmails[$sEmail]))
						{
							if (!$oItem->Auto && isset($aEmails[$sEmail]) && $aEmails[$sEmail] > $oItem->Frequency)
							{
								$oItem->Frequency = $aEmails[$sEmail];
							}
					...
					$mResult = array_merge($mResult, $aTemp);
				}
			}
